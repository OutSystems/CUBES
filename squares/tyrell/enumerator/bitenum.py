...
class Root(Node):
    def __init__(self, enumerator: 'BitEnumerator', id: int) -> None:
        ...
        if enumerator.specification.op_counts:
            self.counters = {}
            for op in enumerator.specification.op_counts.keys():
                var = enumerator.create_variable(f'{op}_counter_{id}', z3.Int)
                enumerator.assert_expr(z3.And(var >= 0, var <=1))
                self.counters[op] = var
...
class BitEnumerator(Enumerator):
    ...
    def create_operator_usage_constraints(self) -> None:
        if self.specification.op_counts is None:
            return

        counters = {}
        for op in self.specification.op_counts.keys():
            var = self.create_variable(f'{op}_counter', z3.Int)
            self.assert_expr(z3.And(var >= 0, var <= self.loc))
            self.assert_expr(var == z3.Sum(*map(lambda x: x.counters[op], self.roots)))
            self.assert_expr(var >= self.specification.op_counts[op])
            counters[op] = var

        for i, root in enumerate(self.roots):
            for func in self.spec.get_function_productions():
                if func.name == 'empty':
                    continue
                current_op = func.name

                preds = []
                for op in self.specification.op_counts.keys():
                    if op == current_op:
                        preds.append(root.counters[op] == 1)
                    else:
                        preds.append(root.counters[op] == 0)
                self.assert_expr(z3.Implies(root.var == func.id, z3.And(*preds)))
