...
class LinesEnumerator(Enumerator):
        ...
        for r in self.roots:
            for c_1 in range(len(r.children)):
                ...
                for c_2 in range(c_1 + 1, len(r.children)):
                    ...
                    self.z3_solver.add(Implies(r.var == prod0.id,
                                               Or(child_1.var != child_2.var, And(child_1.var == 0, child_2.var == 0))))
    ...
    def _resolve_distinct_filters_predicate(self, pred):
        ...
        for r in self.roots:
            self.z3_solver.add(
                Implies(r.var == prod0.id, r.children[int(pred.args[1])].var != r.children[int(pred.args[2])].var))
