...
# times scatter plot
A <- 'original';B <- 'qffd_r_n'; merge(eval(parse(text = A)), eval(parse(text = B)), by='name', suffixes = c("_A", "_B")) %>% filter(timeout_A == 'False' | timeout_B == 'False') %>% ggplot(aes(x=real_A, y=real_B)) + geom_point(color='red', alpha = 0.4, size=2) + scale_x_continuous(trans='log10') + scale_y_continuous(trans='log10') + geom_abline() + geom_hline(yintercept=600, linetype="dashed") + geom_vline(xintercept=600, linetype="dashed") + labs(y=B, x = A) + ggtitle('Real Time')

# process distribution
ggplot(t6_n, aes(x=factor(process))) + geom_bar(fill="turquoise")

#solve time distribution
ggplot(qffd_r_n, aes(x=real)) + geom_histogram()

# solved instances by try
tries <- list('original' = original, 't1' = t1, 't2' = t2, 't3' = t3, 't4' = t4, 't5' = t5, 't6' = t6, 't7' = t7); solved <- bind_rows(tries, .id='try'); results <- factor(solved$status, labels=status_meanings); ggplot(solved, aes(x=try, fill=results)) + geom_bar(position="dodge")
tries <- list('original' = original, 'qffd' = qffd_r_n, 't1' = t1_n, 't2' = t2_n, 't3' = t3_n, 't4' = t4_n, 't5' = t5_n, 't6' = t6_n, 't7' = t7_n); solved <- bind_rows(tries, .id='try'); results <- factor(solved$status, labels=status_meanings); ggplot(solved, aes(x=try, fill=results)) + geom_bar(position="dodge")
tries <- list('original' = original, 't6' = t6, 'qffd_r_n' = qffd_r_n, 't6_n' = t6_n); solved <- bind_rows(tries, .id='try'); results <- factor(solved$status, labels=status_meanings); ggplot(solved, aes(x=try, fill=results)) + geom_bar(position="dodge")
